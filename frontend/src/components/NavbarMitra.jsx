    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-8 lg:px-12 py-3 md:py-4 bg-gradient-to-r from-blue-50 via-white to-blue-50 backdrop-blur-lg bg-opacity-95 border-b border-blue-100 shadow-xl font-sans">
        ...
        <ul className="hidden md:flex gap-6 lg:gap-12">
          <li>
            <NavLink to="/mitra/home" className={navLinkClasses}>
              {icons.home}
              <span className="hidden lg:inline">Home</span>
            </NavLink>
          </li>
          <li>
            <NavLink to="/mitra/orders" className={navLinkClasses}>
              {icons.orders}
              <span className="hidden lg:inline">Orders</span>
            </NavLink>
          </li>
          <li>
            <NavLink to="/mitra/profile" className={navLinkClasses} onClick={handleProfileClick}>
              {icons.profile}
      ...
      {isMenuOpen && (
        <div className="fixed top-16 left-0 right-0 z-40 md:hidden bg-white border-b border-gray-200 shadow-xl backdrop-blur-lg bg-opacity-95">
          <ul className="flex flex-col py-4">
            <li>
              <NavLink to="/mitra/home" className={mobileNavLinkClasses} onClick={() => setIsMenuOpen(false)}>
                {icons.home}
                Home
              </NavLink>
            </li>
            <li>
              <NavLink to="/mitra/orders" className={mobileNavLinkClasses} onClick={() => setIsMenuOpen(false)}>
                {icons.orders}
                Orders
              </NavLink>
            </li>
            <li>
              <NavLink to="/mitra/profile" className={mobileNavLinkClasses} onClick={(e) => { handleProfileClick(e); setIsMenuOpen(false); }}>
                {icons.profile}
                Profile
              </NavLink>
            </li>
